...
def draw_bounding_box_on_image_array(current_frame_number, image,
  ...
  """
  image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
  is_vehicle_detected, csv_line, update_csv,veh = draw_bounding_box_on_image(current_frame_number,image_pil, ymin, xmin, ymax, xmax, color,
                             thickness, display_str_list,
                             use_normalized_coordinates)
  np.copyto(image, np.array(image_pil))
  return is_vehicle_detected, csv_line, update_csv, veh
...
##  print("&&&&&&&&&&&&&&")
  ...
  draw.line([(10, 285), (10, 300), (1250, 300),
             (1250, 285), (10, 285)], width=thickness, fill=color)

  image_temp1 = numpy.array(image)

  roi1 = image_temp1[int(285):int(300), int(10):int(1250)]
  print(roi1)
...
##  if(x_axis[0] == 1):
##    predicted_direction, predicted_speed, is_vehicle_detected, update_csv,name1 = object_counter_x_axis.count_objects_x_axis(top, bottom, right, left,current_frame_number, detected_vehicle_image, ROI_POSITION[0], ROI_POSITION[0]+DEVIATION[0], ROI_POSITION[0]+(DEVIATION[0]*2), DEVIATION[0])

  if(mode_number[0] == 2): #working condition in my case
    predicted_direction,predicted_speed, is_vehicle_detected, update_csv,veh = object_counter.count_objects(roi1,display_str_list[0],top, bottom, right, left, detected_vehicle_image, ROI_POSITION[0], ROI_POSITION[0]+DEVIATION[0], ROI_POSITION[0]+(DEVIATION[0]*2), DEVIATION[0])
    print(veh)
    print("*******************")
    #print(is_vehicle_detected)
    #print(is_vehicle_detected)
...
##    print("*88888")
  ...
  # instead of above.
  if(1 in is_color_recognition_enable):
    speed=int(predicted_speed)
    display_str_list[0] = predicted_direction +" " + predicted_color + " " + display_str_list[0] +" "+ str(speed)
...
def draw_bounding_boxes_on_image_tensors(images,
  ...
  """Draws bounding boxes on batch of image tensorsArgs:
...
#      print(display_str_list)
    ...
    if ((targeted_objects != None) and (display_str_list[0].split(":")[0] in targeted_objects)):
            ...
            is_vehicle_detected, csv_line, update_csv, name1 = draw_bounding_box_on_image_array(current_frame_number,
                image,
                ymin,
                xmin,
                ymax,
  ...
  if(mode == 1):
    ...
    counting_mode = counting_mode.replace("{", "").replace("}", "")+"    "+" Traffic Density:"+str(dense)
    if dense>9:
      engine = pyttsx3.init()

      engine.say("Traffic Congestion")
...
def visualize_boxes_and_labels_on_image_array_y_axis(current_frame_number,
  ...
  for box, color in box_to_color_map.items():
    ...
    temp = ""
    if (("car" in display_str_list[0]) or ("person" in display_str_list[0]) or ("bike" in display_str_list[0]) or ("rikshaw" in display_str_list[0]) or ("bus" in display_str_list[0])):
      #print(display_str_list[0])
      ...
      if ((targeted_objects != None) and (targeted_objects in display_str_list[0])):
              ...
              is_vehicle_detected, csv_line, update_csv, veh = draw_bounding_box_on_image_array(current_frame_number,
                  image,
                  ymin,
                  xmin,
                  ymax,
  ...
  if(mode == 2):
    ...
    return counter, csv_line_util, counting_mode,counter1 , counter2, counter3, counter4

  else:
    return counter, csv_line_util,counter1 , counter2, counter3, counter4
